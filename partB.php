		...
		<header>
			<div class="row">
    			...
    			<div class="col-md-8">
	    			...
	    			<span>DEADLINE</span><p id="deadline"></p><br><br>
	    		</div>
	    		<div class="col-md-2">
			...
			if($_SESSION['id']==$userId)
			{
				...
				if(mysqli_num_rows($resultsfr)==0)
				{
					...
					?>
					<form method="POST" id="sfrB_sys" action="sfrB_sys.php" onsubmit="return confirm('Do you want to submit the form for review?');">
			...
			if($_SESSION['id']==$userId)
			{
				...
				if(mysqli_num_rows($resultssa)==0)
				{
					...
					?>
					<form method="POST" id="ssaB_sys" action="ssaB_sys.php" onsubmit="return confirm('Do you want to submit the form for Self Appraisal?');">
						<input type="hidden" name="year" id="year" value="<?php echo $year; ?>">
...
if($_SESSION['id']==$userId){  
?>  	
<button class="tablink" onclick="window.location.href = 'partA.php?id=<?php echo $_SESSION['id']; ?>&year=<?php echo $_GET['year']; ?>'">PART A</button>
<button class="tablink" onclick="window.location.href = 'partB.php?id=<?php echo $_SESSION['id']; ?>&year=<?php echo $_GET['year']; ?>'">PART B</button>
<?php }
else{
	?>

<button class="tablink" onclick="window.location.href = 'partA.php?id=<?php echo $_GET['id']; ?>&year=<?php echo $_GET['year']; ?>'">PART A</button>
<button class="tablink" onclick="window.location.href = 'partB.php?id=<?php echo $_GET['id']; ?>&year=<?php echo $_GET['year']; ?>'">PART B</button>
<?php
}?>
          <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
		    
		    <a class="nav-item nav-link active" id="nav-cat1-tab" data-toggle="tab" href="#nav-A" role="tab" aria-controls="nav-cat1" aria-selected="true">PART A</a>
		    <a class="nav-item nav-link" id="nav-cat2-tab" data-toggle="tab" href="#nav-cat1" role="tab" aria-controls="nav-cat2" aria-selected="false">Category I</a>
    ...
    <input type="hidden" name="submitted_for_review" id="submitted_for_review" value="<?php echo $sfr_forjs; ?>">
		...
		<div class="tab-pane fade show active" id="nav-A" role="tabpanel" aria-labelledby="nav-cat1-tab"><br>
		<div class="row">
    		<div class="col-md-12">
