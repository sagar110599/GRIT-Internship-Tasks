...
include 'dbh.php';
$depart=mysqli_real_escape_string($conn,$_GET['id']);
			...
			<div class="col offset-md-2" id="part-b-container">
				...
				<table class="table table-bordered" style="background-color: white">
				  	<thead style="color: white">
						...
						for($x=3;$x>0;$x--){
							?>
					    	<th scope="col" rowspan="2" style="background-color: #343a40"><?php echo ($previousyear-$x).'-'.($previousyear-($x-1)); ?></th>
						<?php
						}
						?>
					    </tr>
				  	...
				  	<tbody>
				  		...
				  		if($check=="hod")
				  		{
				  			...
				  			//means hod
				  			$sql="SELECT id, faculty_name, email, date_of_joining, department, hod, committee, principal, admin FROM faculty_table WHERE department='$depart' AND hod=0";
				  			$result=mysqli_query($conn,$sql);

				  		}
				  		elseif ($rowc['hod']&&$rowc['committee']) {
				  			if(isset($_GET['id']))
				  			{
				  				$sql="SELECT id, faculty_name, email, date_of_joining, department, hod, committee, principal, admin FROM faculty_table WHERE department='$depart' AND hod=0";
				  			$result=mysqli_query($conn,$sql);
